...
run_unison () {
    ...
    while [ $status != 0 ]; do
        su - magento2 -c 'unison magento2-code'
#        su - magento2 -c 'unison magento2-var'
        status=$?
    done
}
...
then
    ...
    then
        ...
        then
            ...
            rm -rf /var/www/magento2/status.html
            rm -rf /home/magento2/magento2/status.html
            rm -rf /var/www/magento2/sync-wait
            rm -rf /home/magento2/magento2/sync-wait
            /usr/local/bin/unison.sh &
        else
            (run_unison; /usr/local/bin/unison.sh) &
        fi
    fi
fi
